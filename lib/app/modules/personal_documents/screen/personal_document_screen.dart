...
class PersonalDocumentScreen extends StatelessWidget {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                "Personal Documents",
                style: AppFontStyle.text_24_500(AppTheme.black,
              ...
              Expanded(
                child: Obx(() {
                  ...
                  return RefreshIndicator(
                    onRefresh: () async {
                      await controller.getPersonalDocumentListData();
                    },
              ...
              CustomAnimatedButton(
                  onTap: () {
                    if (controller
                        .listPersonalDocumentData.value.data?.isComplete ==
                        true) {
                      Get.back();
                    } else {
                      CustomSnackBar.show(
                          message:
                          "Please upload all pending personal documents",
          ...
          Obx(() {
            return ListView.separated(
              ...
              itemCount: controller
                  .listPersonalDocumentData.value.data?.data?.length ??
                  0,
              shrinkWrap: true,
              separatorBuilder: (_, __) => WidgetDesigns.hBox(20),
              itemBuilder: (context, index) {
                final doc = controller.documentList[index];
                final completeDocs = controller
                    .listPersonalDocumentData.value.data?.data?[index];
                return documentTypeName(
                  completeDocs?.name ?? "",
                  completeDocs?.isComplete ?? false,
  ...
  Widget documentTypeName(
      String title, bool isCompleted, VoidCallback onTap) {
    return GestureDetector(
      onTap: isCompleted ? null : onTap,
      child: Container(
          decoration: BoxDecoration(
