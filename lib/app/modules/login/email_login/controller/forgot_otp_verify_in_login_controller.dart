  ...
  Future<void> verifyOtpPhone() async {
    ...
    try {
      final data = {
        ...
        "phone": storageServices.getMobile() != '' ? storageServices.getMobile() : phone.value,
        "otp": otpController.text,
      };
      ...
      if (response.status == true) {
        LoadingOverlay().hideLoading();
        CustomSnackBar.show(message: response.message.toString(), color: AppTheme.primaryColor, tColor: AppTheme.white);
        Get.toNamed(Routes.changePasswordInLogin, arguments: {"phone": phone.value,"countryCode":countryCode.value,});
      }
      else if(response.status == false && response.type == 'verifyAndReset'){
        LoadingOverlay().hideLoading();
        updateOtpError(response.message.toString());
